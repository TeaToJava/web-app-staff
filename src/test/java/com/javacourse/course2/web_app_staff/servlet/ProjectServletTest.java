...
class ProjectServletTest {
	...
	@Test
	void test_delete_project() throws ServletException, IOException {
		when(request.getParameter(ID)).thenReturn(uuidAsString);
		when(projectService.delete(randomUUID)).thenReturn(true);
		StringWriter stringWriter = new StringWriter();
		PrintWriter printWriter = new PrintWriter(stringWriter);
		when(response.getWriter()).thenReturn(printWriter);
		projectServlet.doDelete(request, response);
		verify(projectService).delete(randomUUID);
		assertTrue(projectService.delete(randomUUID));
	}
